...
def is_valid_git_repo(path_to_check):
    ...
    output = _get_git_command_output(command, repo_path=path_to_check) # Uses internal helper
    return output == "true"
...
class GitReporter:
    def __init__(self, repo_path, file_line_formatter=None):
        ...
        """
        self.repo_path = os.path.abspath(repo_path) # Store absolute, normalized path
        if not is_valid_git_repo(self.repo_path): # Use public module-level function
            raise ValueError(f"Error: Path '{self.repo_path}' is not a valid Git repository, does not exist, or is not accessible.")
    ...
    def generate_commit_report(self, commit_hash):
        ...
        commit_metadata = _get_git_command_output(metadata_command, repo_path=self.repo_path)
        ...
        commit_message_body_raw = _get_git_command_output(message_body_command, repo_path=self.repo_path)
        has_message = commit_message_body_raw is not None and commit_message_body_raw.strip()

        output_lines.append("") 

        if has_message:
            indented_message_lines = ["    " + line for line in commit_message_body_raw.split('\n')]
        ...
        numstat_output = _get_git_command_output(numstat_command, repo_path=self.repo_path)
        ...
        name_status_output = _get_git_command_output(name_status_command, repo_path=self.repo_path)
        ...
        if name_status_output is not None:
            for line in name_status_output.split('\n'):
                ...
                if status_char in ('A', 'M', 'D', 'T'):
                    change_types_map = {'A': 'added', 'M': 'modified', 'D': 'deleted', 'T': 'modified'}
        ...
        if files_data:
            for path in sorted(files_data.keys()):
                ...
                formatted_line = self.file_line_formatter(path, file_type, data['la'], data['ld'])
...
def _main(): # Renamed from main
    ...
    try:
        ...
        print(f"An unexpected error occurred during GitReporter initialization: {e}", file=sys.stderr)
        parser.print_help(sys.stderr)
        sys.exit(1)
    ...
    if latest_hash:
        ...
        if args.tree: 
            ...
            tree_output = reporter.execute_external_command(
                "tree",                                            # command_name (positional)
                "-L 3",                                            # common_args (positional)
                "-I",                                              # pattern_arg_flag (positional)
                ".git", "node_modules", "*.pyc", "__pycache__"      # *patterns_to_process
            ) 
            if tree_output:
                print(tree_output)
            else:
                print("Could not generate file tree (is 'tree' command installed and accessible?).", file=sys.stderr)
        
        if args.du:
            print("\n" + "="*20 + " DISK USAGE (du) " + "="*20 + "\n")
            du_output = reporter.execute_external_command(
                "du",                                              # command_name (positional)
                "-ch",                                             # common_args (positional)
                "-I",                                      # pattern_arg_flag (positional)
            ...
            if du_output:
                ...
                print("Could not generate disk usage (is 'du' command installed and accessible?).", file=sys.stderr)

        # Removed --tar block
            
    else:
        print(f"Error: Could not find latest commit hash in '{reporter.repo_path}'.", file=sys.stderr)
        sys.exit(1)
